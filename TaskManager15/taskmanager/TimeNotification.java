...
public class TimeNotification {
    ...
    private class timeTask extends TimerTask {
        ...
        public void run() {
            ...
            try {
                ObjectOutputStream output = new ObjectOutputStream(Server.clientSocket.getOutputStream());
                output.writeObject(new Message(1, "NOTIFICATION"));
                output.flush();
                output.writeObject(notificationTask);
                output.flush();
            } catch (IOException e) {
                e.printStackTrace();
            }
